...
class Upload {
  ...
  _startUpload() {
    ...
    if (typeof window.PhoneGap != 'undefined' || typeof window.Cordova != 'undefined' || typeof window.cordova != 'undefined') {
      this._source.slice(start, end, (data) => xhr.send(data), (err) => {
        this._emitError(new DetailedError("tus: could not slice file or stream at start["+start+"] end["+end+"] size["+this._size+"]", err));    
      });
    } else {
