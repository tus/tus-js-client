...
class BaseUpload {
   ...
   */
  static async terminate (url, options = {}) {
    const req = openRequest('DELETE', url, options)

    try {
      const res = await sendRequest(req, null, options)
      ...
      await BaseUpload.terminate(url, newOptions)
    }
  }

  async findPreviousUploads () {
    const fingerprint = await this.options.fingerprint(this.file, this.options)
    return await this._urlStorage.findUploadsByFingerprint(fingerprint)
  }
  ...
  async abort (shouldTerminate) {
    ...
    await BaseUpload.terminate(this.url, this.options)
  ...
  async _createUpload () {
    ...
    try {
      if (this.options.uploadDataDuringCreation && !this.options.uploadLengthDeferred) {
        ...
        res = await this._sendRequest(req, null)
      }
    } catch (err) {
  ...
  async _addChunkToRequest (req) {
    ...
    const { value, done } = await this._source.slice(start, end)
  ...
  async _saveUploadInUrlStorage () {
    ...
    try {
      const urlStorageKey = await this._urlStorage.addUpload(this._fingerprint, storedUpload)
