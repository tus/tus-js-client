...
class BaseUpload {
  constructor (file, options) {
    ...
    // If a stagger is defined, the list of promises that should start when each
    // chunk upload hits the stagger. The promise indices follow the literal order
    // within the file.
    this._chunkStaggerPromises = null

    // If a partial upload, the current chunk index that this partial upload is
    // uploading. The chunk indices follow the literal order within the file.
  ...
  _startParallelUpload () {
    ...
    const chunkStaggerPromises = []
    const chunkStaggerResolves = []
    const chunkStaggerWasResolved = []
    if (staggerPercent !== null) {
      for (let i = 0; i < numChunks; i++) {
        chunkStaggerPromises.push(new Promise((resolve) => chunkStaggerResolves.push(resolve)))
        chunkStaggerWasResolved.push(false)
      }
    }
    let currentChunk = 0
    const uploads = parts.map((part, index) => {
      ...
      return this._source.slice(part.start, part.end)
        .then(({ value }) => new Promise((resolve, reject) => {
          ...
          const numChunksInPart = Math.ceil((partSize) / chunkSize)
          upload.options.onProgress = (newPartProgress) => {
            ...
            if (staggerPercent !== null) {
              ...
              const currentChunk = upload._currentChunk
              console.log('part:', index + 1, '| chunk:', currentChunk + 1, '| chunk percent:', chunkPercent)
              let nextChunk = currentChunk + numChunksInPart
              if (nextChunk >= numChunks) {
                // Went through all parts so increment 1 to start the next chunk.
                nextChunk = (nextChunk % numChunks) + 1
              }
              /* console.log('currentChunk:', currentChunk)
               * console.log('numChunksInPart:', numChunksInPart)
               * console.log('nextChunk:', nextChunk) */

              if (!chunkStaggerWasResolved[nextChunk] && chunkPercent >= staggerPercent) {
                ...
                chunkStaggerResolves[nextChunk]()
                chunkStaggerWasResolved[nextChunk] = true
              }
            }
          }
          ...
          if (staggerPercent !== null) {
            promise = chunkStaggerPromises[currentChunk]
          }
          promise.then(() => upload.start())

          // Increment the current chunk so that the next part starts at the right
          // chunk.
          currentChunk += numChunksInPart
  ...
  _handleUploadResponse (req, res) {
    ...
    if (this._staggerPercent !== null) {
      ...
      promise = this._chunkStaggerPromises[this._currentChunk]
    }

    promise = promise.then(() => this._performUpload())
  }
