...
class BaseUpload {
  constructor (file, options) {
    ...
    // Whether the stagger has been signaled for the current chunk yet. We need this
    // flag so that we only signal the stagger for the next upload once.
    this._chunkStaggerSignaled = false
    ...
    this._staggeredUploadPromise = null

    // The next partial upload that should start when this partial upload reaches the
    // stagger for a chunk.
  ...
  start () {
    ...
    if (this.options.staggerPercent) {
      if (this.options.parallelUploads <= 1) {
  ...
  _startParallelUpload () {
    ...
    parts.forEach((part, index) => {
      ...
      // and only kick off the first one to start with.
      let staggeredUploadPromise
      if (staggerPercent) {
        if (index === 0) {
          // Set the stagger to be already ready for the first part upload.
          staggeredUploadPromise = Promise.resolve()
        } else {
          staggeredUploadPromise = new Promise()
        }
      } else {
        staggeredUploadPromise = Promise.resolve()
      }

      const partialUpload = staggeredUploadPromise
        .then(() => this._source.slice(part.start, part.end))
        .then(({ value }) => new Promise((resolve, reject) => {
          ...
          const options = {
            ...
            onProgress: (newPartProgress) => {
              ...
              // start uploading.
              if (staggerPercent && !this._chunkStaggerSignaled) {
                ...
                if (chunkPercent >= staggerPercent) {
                  ...
                  Promise.resolve(this._nextStaggeredUpload._staggeredUploadPromise)
                  // The stagger has been signaled for this chunk.
                  //
                  // NOTE: We unset this when we start a chunk upload.
                  this._chunkStaggerSignaled = true
                }
              }
            },
      ...
      if (staggerPercent) {
        partialUpload._staggeredUploadPromise = staggeredUploadPromise
      }
      uploads.push(partialUpload)
    })

    if (staggerPercent) {
      // Set the next staggered upload for each partial upload.
      for (let i = 0; i < uploads.length; i++) {
        // The last partial upload should loop back to the first one.
        uploads[i]._nextStaggeredUpload = uploads[(i + 1) % uploads.length]
      }
    }
  ...
  _performUpload () {
    ...
    if (this._staggeredUploadPromise) {
      // We reset the chunk stagger signal when we start a chunk upload.
      this._chunkStaggerSignaled = false
    }
  ...
  _handleUploadResponse (req, res) {
    ...
    let promise
    if (this._staggeredUploadPromise) {
      promise = this._staggeredUploadPromise.then(() => this._staggeredUploadPromise = new Promise())
    } else {
