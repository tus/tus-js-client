...
const debug = Debug("tus-js-client:core");
...
class BaseUpload {
  constructor(file, options) {
    ...
    if ("resume" in options) {
      debug("The `resume` option has been removed in tus-js-client v2. Please use the URL storage API instead.");
    }
  ...
  start() {
    ...
    this.options.fingerprint(file, this.options)
      .then((fingerprint) => {
        if (fingerprint == null) {
          debug("No fingerprint was calculated meaning that the upload cannot be stored in the URL storage.");
        } else {
  ...
  _start() {
    ...
    if (this.url != null) {
      debug(`Resuming upload from previous URL: ${this.url}`);
      this._resumeUpload();
      return;
    }
    ...
    if (this.options.uploadUrl != null) {
      debug(`Resuming upload from provided URL: ${this.options.url}`);
  ...
  _createUpload() {
    ...
    promise.then((res) => {
      ...
      debug(`Created upload at ${this.url}`);
      ...
      // If a fingerprint was calculated for the input (i.e. not a stream), we can store the upload URL.
