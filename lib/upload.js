...
class BaseUpload {
  ...
  static terminate(url, options, cb) {
    ...
    let xhr = openRequest("DELETE", url, options);
  ...
  start() {
    ...
    if (this._source) {
      ...
      this.options.fileReader(file, this.options.chunkSize, (err, source) => {
        if (err) {
          this._emitError(err);
          return;
        }
  ...
  _createUpload() {
    ...
    let xhr = this._openRequest("POST", this.options.endpoint);
    ...
    xhr.onload = () => {
      ...
      this.url = this.options.httpStack.resolveUrl(this.options.endpoint, location);
   ...
   */
  _resumeUpload() {
    let xhr = this._openRequest("HEAD", this.url);
  ...
  _startUpload() {
    ...
    if (this.options.overridePatchMethod) {
      xhr = this._openRequest("POST", this.url);
      xhr.setRequestHeader("X-HTTP-Method-Override", "PATCH");
    } else {
      xhr = this._openRequest("PATCH", this.url);
    }
  ...
  _openRequest(method, url) {
    const xhr = openRequest(method, url, this.options);
    this._xhr = xhr;
    return xhr;
  }
...
function openRequest(method, url, options) {
  const xhr = options.httpStack.createRequest();
