...
const defaultOptions = {
  ...
  parallelUploads            : 1,
  // If set, wait for one partial parallel upload to reach this percentage
  // before starting the next partial upload.
  parallelUploadsStaggerPercent : null,
...
class BaseUpload {
  ...
  start () {
    ...
    if (this.options.parallelUploadsStaggerPercent) {
      if (this.options.parallelUploads <= 1) {
        this._emitError(new Error(`tus: cannot use the parallelUploadsStaggerPercent option when parallelUploads is disabled`))
      }
    }
  ...
  _startParallelUpload () {
    ...
    let totalProgress = 0
    const parallelUploadsStaggerPercent = this.options.parallelUploadsStaggerPercent;
    ...
    let prevUploadStaggerReady = Promise.resolve()
    parts.forEach((part, index) => {
      let lastPartProgress = 0
      const partSize = part.end - part.start
      // Promise that signals when the stagger is ready for the next part upload.
      let uploadStaggerResolve
      const uploadStaggerReady = new Promise((resolve) => uploadStaggerResolve = resolve)

      const partialUpload = prevUploadStaggerReady
        ...
        .then(({ value }) => new Promise((resolve, reject) => {
          ...
          const options = {
            ...
            onProgress: (newPartProgress) => {
              ...
              // If a stagger is set, signal to the next upload when it can
              // start uploading.
              const newPartPercent = (newPartProgress / partSize) * 100
              console.log('part:', index+1, '| percent:', newPartPercent);
              if (parallelUploadsStaggerPercent && uploadStaggerResolve &&
                  newPartPercent >= parallelUploadsStaggerPercent) {
                console.log("Reached stagger")
                uploadStaggerResolve()
                uploadStaggerResolve = null;
              }
            },
      ...
      if (parallelUploadsStaggerPercent) {
        prevUploadStaggerReady = uploadStaggerReady
      }
    })
