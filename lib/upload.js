...
class BaseUpload {
  constructor (file, options) {
    ...
    // The promise that is resolved to signal that this upload should start. It is
    // resolved either if we are doing the first chunk of the first upload, or when
    // the previous upload reaches a chunk stagger.
    this._staggeredUploadResolve = null

    // If a stagger is defined, the next upload that should start when this upload
    // hits the stagger.
    this._nextStaggeredUpload = null
  ...
  _startParallelUpload () {
    ...
    const uploadsReadyResolve = new Array(parts.length)
    const uploadsReady = parts.map((_part, index) => new Promise((resolve) => uploadsReadyResolve[index] = resolve))
    const uploads = parts.map((part, index) => {
      ...
      return this._source.slice(part.start, part.end)
        .then(({ value }) => new Promise((resolve, reject) => {
          // If the stagger is set, start a stagger promise for each partial upload.
          //
          // NOTE: We resolve the promise for the first partial upload later, when all
          // uploads are ready.
          let staggeredUploadResolve
          const staggeredUploadPromise = new Promise((promiseResolve) => staggeredUploadResolve = promiseResolve)
          if (staggerPercent === null) {
            staggeredUploadResolve()
          }
          ...
          upload.options.onProgress = (newPartProgress) => {
            ...
            if (staggerPercent !== null) {
              ...
              // current part size.
              const chunkPercent = ((newPartProgress % chunkSize) / chunkSize) * 100
              console.log('part:', index + 1, '| chunk percent:', chunkPercent)

              // NOTE: keep signaling the stagger even when it's already been
              // signaled. This prevents some pathological edge cases.
              if (chunkPercent >= staggerPercent) {
                console.log('Reached stagger')
                // The next partial upload that should start when this partial upload
                // reaches the stagger for a chunk.
                upload._nextStaggeredUpload._staggeredUploadResolve()
              }
            }
          }
          if (staggerPercent !== null) {
            upload._staggeredUploadResolve = staggeredUploadResolve
          }
          upload._initialUpload = false

          staggeredUploadPromise.then(() => upload.start())

          // Store the uploads in an array, so we can later abort them if necessary.
          this._parallelUploads.push(upload)

          if (staggerPercent !== null) {
            // Signal that this upload is ready to start.
            console.log('resolving index:', index)
            uploadsReadyResolve[index]()
          }
        }))
    })

    if (staggerPercent !== null) {
      // Wait for all uploads to be ready to start.
      Promise.all(uploadsReady).then(() => {
        // Set the next staggered upload for each upload.
        const parallelUploads = this._parallelUploads
        console.log('parallelUploads:', parallelUploads)
        for (let i = 0; i < parallelUploads.length; i++) {
          parallelUploads[i]._nextStaggeredUpload = parallelUploads[(i + 1) % parallelUploads.length]
        }

        // Kick off the first upload.
        parallelUploads[0]._staggeredUploadResolve()
      })
    }
  ...
  abort (shouldTerminate) {
    // Count the number of arguments to see if a callback is being provided in
    // the old style required by tus-js-client 1.x, then throw an error if it is.
    // `arguments` is a JavaScript built-in variable that contains all of the
  ...
  _handleUploadResponse (req, res) {
    ...
    if (this._staggerPercent !== null) {
      // Signal to the next upload one more time, to avoid some rare edge cases.
      this._nextStaggeredUpload._staggeredUploadResolve()
    }
    ...
    // Not done uploading yet. If staggered uploads are enabled, pause here until the previous
    // upload is at the stagger percent.
    let promise = Promise.resolve()
    if (this._staggerPercent !== null) {
      promise = new Promise((resolve) => { this._staggeredUploadResolve = resolve }).then(() => console.log('promise triggered'))
    }
