...
class BaseUpload {
  ...
  _startParallelUpload () {
    ...
    // upload is completed.
    const firstChunk = buildChunkStaggers(totalSize, parts.length, chunkSize)
    ...
    const uploads = parts.map((part, index) => {
      ...
      return this._source.slice(part.start, part.end)
        .then(({ value }) => new Promise((resolve, reject) => {
          ...
          // Update the current chunk so that the next part starts at the right
          // chunk.
          currentChunk = currentChunk.nextChunkStagger
...
function buildChunkStaggers (totalSize, partCount, chunkSize) {
  ...
  const partsChunks = new Array(partCount).fill([])
  ...
  function newChunk (index) {
    ...
    const partChunks = partsChunks[index]
