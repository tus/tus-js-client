...
class Upload {
  ...
  static terminate(url, options, cb) {
    ...
    let xhr = openRequest("DELETE", url);
  ...
  _createUpload() {
    ...
    let xhr = openRequest("POST", this.options.endpoint);
   ...
   */
  _resumeUpload() {
    let xhr = openRequest("HEAD", this.url);
  ...
  _startUpload() {
    ...
    if (this.options.overridePatchMethod) {
      xhr = openRequest("POST", this.url);
      xhr.setRequestHeader("X-HTTP-Method-Override", "PATCH");
    } else {
      xhr = openRequest("PATCH", this.url);
    }
