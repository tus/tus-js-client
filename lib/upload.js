  ...
  _startUpload() {
    ...
    if ("upload" in xhr) {
      ...
      xhr.upload.onloadstart = (e) => {
        ...
        this._emitProgress(start, this._size);
      };
    }
    ...
    let chunk = this._source.slice(start, end);

    if(chunk instanceof Promise) {
      chunk
        .then(data => xhr.send(data))
        .catch(err => this._emitError(new DetailedError("tus: could not slice file or stream at start["+start+"] end["+end+"] size["+this._size+"]", err)));
    } else {
      xhr.send(chunk);
    }
  }
