...
class Request {
  ...
  send(body) {
    ...
    if (this._agent instanceof Object && this._agent.protocol != options.protocol) {
      const err = `Cannot use ${this._agent.protocol} Agent for ${options.protocol} request`;
    ...
    req.on("response", (res) => {
      ...
      res.resume();   // this allows Agent to reuse same connection for next HTTP request, as suggested in https://github.com/nodejs/node/issues/9595
    });
